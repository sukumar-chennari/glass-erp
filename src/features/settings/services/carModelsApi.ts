      ...
      ...mockableQuery<CarModel[], { brandId?: string; status?: CarModelStatus; search?: string } | void>({
        ...
        transformResponse: (raw) =>
          normalizeArray<Record<string, unknown>>(raw).map((m) => ({
            ...
            status:       m['status'],
            cc:           (m['cc'] as number | null | undefined) ?? null,
            ccCondition:  (m['ccCondition'] as string | null | undefined) ?? null,
            bodyType:     Array.isArray(m['bodyType']) ? (m['bodyType'] as string[]) : [],
            brand:        m['brand'] as { id: string; name: string } | undefined,
          })) as CarModel[],
      }),
